...
RAW_DATA_DIR = "/root/autodl-tmp/Albert/project-neo/raw_data"


def prepare(
    destination_path: Path = Path(RAW_DATA_DIR),
    tokenizer_path: Path = Path("/root/autodl-tmp/Albert/model/chinese-llama-2-7b-hf/tokenizer.model"),
    ...
    file_paths = []
    for file_path in Path(raw_data_dir).iterdir():
        file_paths.append(file_path)
    
    if len(file_paths) == 0:
        raise AssertionError(f"{raw_data_dir} is empty!")
    
    data = []
    print("We got {} raw txts, processing raw data...".format(len(file_paths)))
